...
- **Student performance joins on `institute_name` + `semester` (+ `section`).** `student_performance` carries the canonical `institute_name` (added at build time) and a `S-0NN` section that matches delivery. It is **many rows per section with no course key**, so aggregate to `student_perf_by_section` / `student_perf_by_college` before joining — one raw row is a single practice block, not the section — and recompute rates from summed counts.
...
| `student_performance` | one row per practice block (**un-keyed within a section**) | **MCQ & coding practice outcomes** — attendance, attempts, accuracy, completion — for 15 colleges, Sem 1-2. **Many rows per (college, semester, section) with NO course/date column**, so a single row is one practice block, **not** a section total — never read one row as "the section". Rates are per-row; **aggregate and recompute from summed counts, never average**. Carries canonical `institute_name` (added by `build_student_performance.py` via a curated crosswalk); section is `S-0NN`. |
| `student_perf_by_section` | one row per college × semester × section | **Use this for per-section MCQ/coding**, not the raw table: summed attempts/correct/completions + recomputed `mcq_attempt_pct`, `mcq_accuracy_pct`, `coding_attempt_pct`, `coding_completion_pct`. `*_attendance_pct` is a **practice-weighted mean** (attendance is a per-block student count, not additive). |
| `student_perf_by_college` | one row per college × semester | Same measures rolled to college×semester — use for cross-college comparison ("which college has the best MCQ accuracy / coding completion in Sem 2"). |
